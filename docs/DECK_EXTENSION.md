...
Heavy generators (slicer, granular, multi-stage graphs) use **`gen_block`** in deck. The core lexer/parser collects lines until `end gen_block` and passes them to **`TplExtension.parseGenBlock`** in [`src/tpl/TplExtension.tish`](../src/tpl/TplExtension.tish).
